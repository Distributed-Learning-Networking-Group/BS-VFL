...
from src.torchmimic.data.preprocessing import Discretizer, Normalizer
from src.torchmimic.data.readers import InHospitalMortalityReader
from src.torchmimic.data.base_dataset import BaseDataset
...
class IHMDataset(BaseDataset):
    ...
    def __init__(
        ...
        listfile = "train\\listfile.csv" if train else "test\\listfile.csv"

        self._read_data(root, listfile=listfile)
    ...
    def _read_data(self, root, listfile):
        self.reader = InHospitalMortalityReader(
            dataset_dir=os.path.join(root, "train") if listfile.startswith('train') else os.path.join(root, "test"),
        ...
        normalizer_state = os.path.join(
            ...
            "ihm_ts1.0.input_str_previous.start_time_zero.normalizer",
        )
