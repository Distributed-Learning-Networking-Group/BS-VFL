from src.torchmimic.metrics import MetricMeter, AUCROC, aucpr
...
class IHMLogger(BaseLogger):
    ...
    def update(self, outputs, labels, loss):
        ...
        self.metrics["Loss"].update(loss.item(), batch_size)  # "Loss": AverageMeter()
        self.metrics["AUC-ROC"].update(label_tmp, outputs[:, 1])
        self.metrics["AUC-PR"].update(label_tmp, outputs[:, 1])
        # self.metrics["accuracy"].update(label_tmp, outputs.argmax(axis=1))
